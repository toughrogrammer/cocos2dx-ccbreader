	    ...
	    case kCCBPropTypeFloatXY:
	      {
			...
			case kCCBPropTypeColor4: 
            {
                ccColor4F color4F = this->parsePropTypeColor4(pNode, pParent, pCCBReader, propertyName.c_str());
                if(setProp) 
                {
                    this->onHandlePropTypeColor4(pNode, pParent, propertyName.c_str(), color4F, pCCBReader);
                }
                break;
            }
            ...
            case kCCBPropTypeString: {
				...
				bool localized = pCCBReader->readBool();
				if( localized )
				{
...
CCPoint CCNodeLoader::parsePropTypePosition(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) {
	...
	int corner = pCCBReader->readByte();
	int xUnit = pCCBReader->readByte();
	int yUnit = pCCBReader->readByte();
    
    CCSize containerSize = pCCBReader->getAnimationManager()->getContainerSize(pParent);
    
    CCPoint pt = getAbsolutePosition( ccp( x, y ), 
		(CCPositionUnit)xUnit, (CCPositionUnit)yUnit,
		corner, 
		containerSize, 
		pPropertyName);
    ...
    if (pCCBReader->getAnimatedProperties()->find(pPropertyName) != pCCBReader->getAnimatedProperties()->end())
    {
        CCArray *baseValue = CCArray::create(CCBValue::create(x),
                                             CCBValue::create(y),
                                             CCBValue::create(corner),
											 CCBValue::create(xUnit),
											 CCBValue::create(yUnit),
                                             NULL);
...
CCSize CCNodeLoader::parsePropTypeSize(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
	...
	int widthUnit = pCCBReader->readByte();
	int heightUnit = pCCBReader->readByte();
    ...
    {
        size.width = CC_CONTENT_SCALE_FACTOR() * width;
    }
    else if (widthUnit == CCSizeUnitNormalized)
    {
        size.width = width * containerSize.width;
    }
    else if (widthUnit == CCSizeUnitInsetPoints)
    {
        size.width = containerSize.width - width;
    }
    else if (widthUnit == CCSizeUnitInsetUIPoints)
    {
        size.width = containerSize.width - width * CC_CONTENT_SCALE_FACTOR();
    }
    ...
    {
        size.height = CC_CONTENT_SCALE_FACTOR() * height;
    }
    else if (heightUnit == CCSizeUnitNormalized)
    {
        size.height = height * containerSize.height;
    }
    else if (heightUnit == CCSizeUnitInsetPoints)
    {
        size.height = containerSize.height - height;
    }
    else if (heightUnit == CCSizeUnitInsetUIPoints)
    {
        size.height = containerSize.height - height * CC_CONTENT_SCALE_FACTOR();
    }
...
CCSpriteFrame * CCNodeLoader::parsePropTypeSpriteFrame(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) 
{
	...
	std::string filepath = pCCBReader->readCachedString();
...
// Below function is no longer used because color data is float type now.
...
ccColor4F CCNodeLoader::parsePropTypeColor4(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName)
{
	float r = pCCBReader->readFloat();
	float g = pCCBReader->readFloat();
	float b = pCCBReader->readFloat();
	float a = pCCBReader->readFloat();
    
	ccColor4F color = { r, g, b, a };
    if (pCCBReader->getAnimatedProperties()->find(pPropertyName) != pCCBReader->getAnimatedProperties()->end())
    {
        ccColor4FWrapper *value = ccColor4FWrapper::create(color);
        pCCBReader->getAnimationManager()->setBaseValue(value, pNode, pPropertyName);
    }
    return color;
}
...
void CCNodeLoader::onHandlePropTypeColor4(CCNode * pNode, CCNode * pParent, const char* pPropertyName, ccColor4F pCCColor4F, CCBReader * pCCBReader) {
	CCNodeRGBA *rgbaNode = dynamic_cast< CCNodeRGBA* >( pNode );
	if( rgbaNode != NULL )
	{
		...
		rgbaNode->setColor( ccc3( r, g, b ) );
		rgbaNode->setOpacity( a );
	}
	else
	{
		ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
