...
int CCBReader::readInt(bool pSigned) {
   unsigned int v = *(unsigned int *)(this->mBytes + this->mCurrentByte);
    int numBits = 32;
    int extraByte = 0;
    v &= -((int)v);
    if (v) numBits--;
    ...
    if ((numBits & 0x00000007) == 0)
    {
        extraByte = 1;
		this->mCurrentBit = 0;
		this->mCurrentByte += (numBits >> 3);
    }
    else
    {
		this->mCurrentBit = numBits - (numBits >> 3) * 8;
		this->mCurrentByte += (numBits >> 3);
    }
	...
	unsigned char prefix = REVERSE_BYTE(*(this->mBytes + this->mCurrentByte)) & prefixMask[this->mCurrentBit];
    long long current = prefix;
    int numBytes = 0;
    int suffixBits = (numBits - (8 - this->mCurrentBit) + 1);
    if (numBits >= 8)
    {
        suffixBits %= 8;
        numBytes = (numBits - (8 - (int)(this->mCurrentBit)) - suffixBits + 1) / 8;
    }
    if (suffixBits >= 0)
    {
		this->mCurrentByte++;
        ...
        {
            current <<= 8;
            unsigned char byte = REVERSE_BYTE(*(this->mBytes + this->mCurrentByte));
            current += byte;
			this->mCurrentByte++;
        }
        current <<= suffixBits;
        unsigned char suffix = (REVERSE_BYTE(*(this->mBytes + this->mCurrentByte)) & suffixMask[suffixBits]) >> (8 - suffixBits);
        current += suffix;
    }
    else
    {
