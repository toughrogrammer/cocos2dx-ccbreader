...
CCActionInterval* CCBAnimationManager::getAction(CCBKeyframe *pKeyframe0, CCBKeyframe *pKeyframe1, const char *pPropName, CCNode *pNode)
{
    ...
    else if (strcmp(pPropName, "position") == 0)
    {
		...
		int corner = ((CCBValue*)value->objectAtIndex(2))->getIntValue();
		CCPositionUnit xUnit = (CCPositionUnit)((CCBValue*)value->objectAtIndex(3))->getIntValue();
		CCPositionUnit yUnit = (CCPositionUnit)((CCBValue*)value->objectAtIndex(4))->getIntValue();
        
        CCSize containerSize = getContainerSize(pNode->getParent());
        
        CCPoint absPos = getAbsolutePosition( ccp( x, y ), 
